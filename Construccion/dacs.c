...
ushort * optimizationk(uint * list,uint listLength, uint * nkvalues){
...
//	printf("optimizationk\n");
	kvalues = optimizationk(list,listLength,&nkvalues);
/*	int a;
	for(a=0; a<nkvalues; a++){
		printf("Level %u val -> %u\n", a, kvalues[a]);
	}
*/	
	uint kval;
	uint oldval =0;
...
/*-----------------------------------------------------------------
 ...
 ---------------------------------------------------------------- */
void saveFTFlex(FTRep * rep, FILE * flist){
	//FILE * flist;
	//flist = fopen(filename,"w");
	fwrite(&(rep->listLength),sizeof(uint),1,flist);
	fwrite(&(rep->nLevels),sizeof(byte),1,flist);
